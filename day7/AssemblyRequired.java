...
public class AssemblyRequired {
  ...
  private static HashMap<String, String> wires2 = new HashMap<String, String>();
  ...
  private static int evaluate( String wire ) {
    ...
    if ( wireValue != null ) {
      ...
      System.out.format( "Evaluating wire %s\n", wire );

      String wireDefinition = wires.get( wire );
      
      System.out.format( "Got definition %s for wire %s\n\n", wireDefinition, wire ); 
      ...
      if ( andMatcher.find() ) {
        int r1 = evaluate( andMatcher.group( 1 ) );
        int r2 = evaluate( andMatcher.group( 2 ) );
        wires.put( andMatcher.group( 1 ), String.valueOf( r1 ) );
        wires.put( andMatcher.group( 2 ), String.valueOf( r2 ) );
        return and( r1, r2 );
      } else if ( orMatcher.find() ) {
        int r1 = evaluate( orMatcher.group( 1 ) );
        int r2 = evaluate( orMatcher.group( 2 ) );
        wires.put( orMatcher.group( 1 ), String.valueOf( r1 ) );
        wires.put( orMatcher.group( 2 ), String.valueOf( r2 ) );
        return or( r1, r2 );
      } else if ( lshiftMatcher.find() ) {
        int r1 = evaluate( lshiftMatcher.group( 1 ) );
        int r2 = evaluate( lshiftMatcher.group( 2 ) );
        wires.put( lshiftMatcher.group( 1 ), String.valueOf( r1 ) );
        wires.put( lshiftMatcher.group( 2 ), String.valueOf( r2 ) );
        return lshift( r1, r2 );
      } else if ( rshiftMatcher.find() ) {
        int r1 = evaluate( rshiftMatcher.group( 1 ) );
        int r2 = evaluate( rshiftMatcher.group( 2 ) );
        wires.put( rshiftMatcher.group( 1 ), String.valueOf( r1 ) );
        wires.put( rshiftMatcher.group( 2 ), String.valueOf( r2 ) );
        return rshift( r1, r2 );
      } else if ( notMatcher.find() ) {
        int r = evaluate( notMatcher.group( 1 ) );
        wires.put( notMatcher.group( 1 ), String.valueOf( r ) );
        return not( r );
  ...
  private static void part1( BufferedReader bf ) {
    ...
    System.out.format( "Evaluation time (minutes): %f\n", durationMinutes );
  }
  ...
  private static void part2( BufferedReader bf ) {
    ...
    wires2.putAll( wires );
    wires2.put( "b", String.valueOf( evaluate( "a" ) ) );
    wires.putAll( wires2 );
    System.out.format( "a: %d\n", evaluate( "a" ) );
  }
